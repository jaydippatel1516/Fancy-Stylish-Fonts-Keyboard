...
public class FancyFontsAdapter extends RecyclerView.Adapter<FancyFontsAdapter.Holder> {
    ...
    public FancyFontsAdapter(Context context, OnFontInstall onFontInstall2) {
        ...
        while (it.hasNext()) {
            Object next = it.next();
            if (next instanceof SignMoji) {
                this.interfaceList.remove(next);
            }
            if (next instanceof SignMoji2) {
                this.interfaceList.remove(next);
            }
            if (next instanceof SignMoji3) {
                this.interfaceList.remove(next);
            }
            if (next instanceof SignMoji4) {
                this.interfaceList.remove(next);
            }
            if (next instanceof SignMoji5) {
                this.interfaceList.remove(next);
            }
            if (next instanceof SignMoji6) {
                this.interfaceList.remove(next);
            }
            if (next instanceof SignMoji7) {
                this.interfaceList.remove(next);
            }
            if (next instanceof SignMoji8) {
                this.interfaceList.remove(next);
            }
            if (next instanceof SignMoji9) {
                this.interfaceList.remove(next);
            }
            if (next instanceof SignMoji10) {
                this.interfaceList.remove(next);
            }
            if (next instanceof SignMoji11) {
                this.interfaceList.remove(next);
            }
            if (next instanceof SignMoji12) {
                this.interfaceList.remove(next);
            }
        }
    }
